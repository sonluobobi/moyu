...
TEMP_DIR=${OLD_PATH}/temp_${SERV}/
rm -rf $TEMP_DIR
mkdir $TEMP_DIR

if [ "${NOTUNIQ_NAME}" != "" ] && [ "${NOTUNIQ_NAME}" == "true" ] ; then
	FILE_NAME="servers_dist_$SERV.tar.gz"
else
	FILE_NAME="servers_dist.tar.gz"
fi
...
rm -rf $SERV/www/$WEB_SERVER/data/*
...
if [ -d "/data/moyu/www/default/" ] ;then
	echo "拷贝www目录...."
	cp -rd /data/moyu/www/default $SERV/www/
else
	echo "/data/moyu/www下面没有default目录有错误！"	
	exit
fi
...
##增加publish目录的打包
if [[ "$ISINSTALLNEW" != "" ]] && [[ "$ISINSTALLNEW" == "true" ]] ; then
	cp -r /data/moyu/publish .
	tar -czvf $FILE_NAME publish bin conf lib run from_serv_etc backup_db $SERV/bin $SERV/www/default $SERV/conf $SERV/lib $SERV/www/$GAME_SERVER $SERV/www/$WEB_SERVER $EXCLUDE >/dev/null 2>&1
else
	tar -czvf $FILE_NAME bin conf lib run from_serv_etc backup_db $SERV/bin $SERV/www/default $SERV/conf $SERV/lib $SERV/www/$GAME_SERVER $SERV/www/$WEB_SERVER $EXCLUDE >/dev/null 2>&1
fi
