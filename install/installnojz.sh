...
if [ $IS_INSTALLED_OTHER_SERV -eq 0 ];then
	...
	echo -e "hosts allow = 42.62.107.61  10.22.107.61 127.0.0.1 10.22.23.81 42.62.23.81  \n">> /etc/rsyncd.conf
...
echo $ARENA_ID $PROXY_PORT $WEB_SERVER $SERVER_TYPE $BASE_SERVER_EXTERNAL_IP
PHP_TOKEN=`php -r "echo md5('$ARENA_ID$PROXY_PORT$ADD_SERVER_TOKEN$WEB_SERVER$SERVER_TYPE$BASE_SERVER_EXTERNAL_IP');"`
echo PHP_TOKEN:$PHP_TOKEN
PHP_RET=`curl -Ss "http://pkadmin$DOMAIN_SUFFIX/interface_pk_server.php?aid=$ARENA_ID&port=$PROXY_PORT&domain=$WEB_SERVER&type=$SERVER_TYPE&ip=$BASE_SERVER_EXTERNAL_IP&s=$PHP_TOKEN" `
echo $PHP_RET
if [ "$PHP_RET" != "succ"  ];then
	ERROR "自动添加游戏服列表的地址失败,请联系刘建筑!"
fi
echo http://pkadmin$DOMAIN_SUFFIX/interface_pk_server.php?aid=$ARENA_ID\&port=$PROXY_PORT\&domain=$WEB_SERVER\&type=$SERVER_TYPE\&ip=$BASE_SERVER_EXTERNAL_IP\&s=$PHP_TOKEN
