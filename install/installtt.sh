...
judge_dir_or_file -f "/usr/lib64/liblua.so" "请手动建liblua.so软链接,如mv /usr/lib64/liblua-5.1.so /usr/lib64/libluaso ,ln -s /usr/lib64/libluaso /usr/lib64/liblua.so,ldconfig"
...
let is_exist_proxy_port=`netstat -nap | grep "[0-9]:$PROXY_PORT[\ \t]" -c`
...
if [ $IS_INSTALLED_OTHER_SERV -eq 1 ];then
	...
	if [ -d "./publish" ] ; then
		...
		##特殊替换不可使用sed_rep_add
		sed -i "s/$FROM_DOMAIN_SUFFIX/$DOMAIN_SUFFIX/g" `grep $FROM_DOMAIN_SUFFIX -rl ${MOYU_ROOT}/publish`
...
if [ $IS_INSTALL_BASE -eq 1 ];then
	...
	if [ $ProxyServerType -eq 3  ];then
		...
		sed_rep_add "${FROM_WEB_SERVER}" "${WEB_SERVER}" etc/$CROND_FILE
		sed_rep_add "moyu/$FROM_SERV" "moyu/$SERV" etc/$CROND_FILE
		sed_rep_add "$FROM_SERV" "$SERV" etc/$CROND_FILE
...
if [ $IS_INSTALLED_OTHER_SERV -eq 0 ];then
	...
	echo "${region_host} ${V3}com" >> ${ETC_HOSTS_FILE}
...
if [ $IS_INSTALL_PROXY -eq 1 ];then
	...
	if [ $IS_INSTALLED_OTHER_SERV -eq 1 ];then		
		...
		echo -e "	listen 80;" >> $NGINX_SERVERS_CONF
		echo -e "	include /data/moyu/conf/nginx.location.conf;" >> $NGINX_SERVERS_CONF
		echo -e "	root /usr/share/nginx/html;" >> $NGINX_SERVERS_CONF
		echo -e "	deny all;" >> $NGINX_SERVERS_CONF
...
if [ $IS_INSTALLED_OTHER_SERV -eq 0 ];then
	...
	echo -e "hosts allow = 42.62.107.61  10.22.107.61 127.0.0.1 10.22.23.81 52.77.108.4 123.206.209.68  \n">> /etc/rsyncd.conf
...
then	
	...
	sed_rep_add "/$FROM_SERV.*com/" "/$WEB_SERVER/"  ${MOYU_WWW_GAME_WEB_INCLUDE_CONFIG_FILE}
...
sed_rep_add "${FROM_SVR}" "${SERV}"  ${MOYU}/bin/auto_update_pklist.sh
...
if [ 1 -eq 1 ];then
	...
	if [ "$PHP_RET" != "SUCCESS"  ];then
		COMM "server-list: add server $ADD_SERVER_NAME fail, please concat RTX:6006 to add again! thanks! BASE_SERVER_EXTERNAL_IP = $BASE_SERVER_EXTERNAL_IP , relate to "
	...
	if [ "$PHP_RET" != "SUCCESS"  ];then
		WARN "server-list: check server $ADD_SERVER_NAME  fail, please concat RTX:6006 to add again! thanks! BASE_SERVER_EXTERNAL_IP = $BASE_SERVER_EXTERNAL_IP , relate to "
		for line_t in $PHP_RET; do COMM $line_t; done
	fi
...
PHP_RET=`curl -Ss "http://pkadmin$DOMAIN_SUFFIX/interface_pk_server.php?aid=$ARENA_ID&port=$PROXY_PORT&domain=$WEB_SERVER&type=$SERVER_TYPE&ip=$BASE_SERVER_EXTERNAL_IP&s=$PHP_TOKEN&pkid=$TargetPkServerAid" `
echo $PHP_RET
if [ "$PHP_RET" != "succ"  ];then
	ERROR "自动添加游戏服列表的地址失败,请联系刘建筑!"
fi
echo http://pkadmin$DOMAIN_SUFFIX/interface_pk_server.php?aid=$ARENA_ID\&port=$PROXY_PORT\&domain=$WEB_SERVER\&type=$SERVER_TYPE\&ip=$BASE_SERVER_EXTERNAL_IP\&s=$PHP_TOKEN\&pkid=$TargetPkServerAid
...
################################ 增加kill后的检测begin ##############################
INFO "增加kill自启逻辑"
isfile_hasstring "^BeamNames=(.*[\t\ ]$SERV[\t\ ].*" $MOYU_ROOT/bin/autocksrv.sh
rett=$?
if [[ rett -eq 1 ]]; then
	sed_rep_add "^BeamNames=(" "BeamNames=( $SERV " $MOYU_ROOT/bin/autocksrv.sh
fi
